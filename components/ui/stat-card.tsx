...
export function StatCard({ title, value, icon: Icon, onClick, colorClass }: StatCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white p-4 rounded-xl shadow-sm border border-slate-100 transition-all ${isClickable ? 'cursor-pointer hover:shadow-md hover:border-[hsl(var(--color-brand))]/20 group' : ''}`}
    >
